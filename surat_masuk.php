...
$data_surat_masuk = $db_koneksi->query("SELECT * FROM surat WHERE jenis = 'masuk'");
    ...
    <div class="card m-2">
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-hover table-bordered" id="tabel_surat">
                ...
                <?php
                $index = 0;
                if (mysqli_num_rows($data_surat_masuk) > 0) {
                    while ($row = $data_surat_masuk->fetch_row()) {
                        echo "<tr>";
                        echo "<td><input type='text' name='$row[0]' hidden>" . ($index + 1) . "</td>";
                        echo "<td>" . $row[2] . "</td>";
                        echo "<td>" . $row[3] . "</td>";
                        echo "<td>" . $row[4] . "</td>";
                        echo "<td>" . $row[5] . "</td>";
        ...
        function exportToPDF() {
            ...
            doc.autoTable({
                ...
                didDrawPage: function(data) {
                    doc.text("SURAT MASUK", 225, 30);
                },
            })
            doc.save('Surat Masuk.pdf')
        }
    </script>
</body>
